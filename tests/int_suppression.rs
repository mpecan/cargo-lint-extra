...
fn test_suppression_inline_line_length() {
    ...
    let diags = test_helpers::run_on_fixture("suppressed.rs", "suppressed-inline", &config);
...
fn test_suppression_next_line_todo() {
    let config = Config::default();
    let diags = test_helpers::run_on_fixture("suppressed.rs", "suppressed-todo", &config);
...
fn test_suppression_block_covers_function() {
    let config = Config::default();
    let diags = test_helpers::run_on_fixture("suppressed.rs", "suppressed-block", &config);
