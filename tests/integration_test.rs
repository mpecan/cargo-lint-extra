...
use cargo_lint_extra::config::{
    CloneDensityConfig, Config, FileLengthConfig, LineLengthOverride, TestConfig,
    TestRulesOverrides,
};
...
// --- Clone density integration tests ---

#[test]
fn test_clone_density_detected() {
    let config = Config::default();
    let diags = run_on_fixture("clone_density.rs", "clone-density", &config);
...
fn test_clone_density_disabled() {
    ...
    let diags = run_on_fixture("clone_density.rs", "clone-density-disabled", &config);
    assert!(
...
fn test_clone_density_custom_thresholds() {
    ...
    let diags = run_on_fixture("clone_density.rs", "clone-density-relaxed", &config);
    let clone_diags: Vec<_> = diags.iter().filter(|d| d.rule == "clone-density").collect();
    assert!(
